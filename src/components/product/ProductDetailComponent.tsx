...
import z from "zod";
import { productRes } from "./ProductCardComponent";
...
const ProductDetail = ({ className, id }: ProductDetail1Props) => {
  ...
  useEffect(() => {
    const getSingleProductById = async () => {
      ...
      const getData = await fetch(
        `${process.env.NEXT_PUBLIC_FAKESTORE_API}/products/${productId}`,
      );
  ...
  if (!singleProduct) return <div className="py-32 text-center">Loading...</div>;
  ...
  const PRODUCT_DETAILS = {
    ...
    reviews: {
      ...
      totalReviewers: `${singleProduct.rating.count}`,
    },
    description: singleProduct.description,
    price: {
      regular: singleProduct.price,
      currency: "USD",
    },
    hinges: {
      size: {
    ...
    images: [
      {
        srcset: `${singleProduct.image} 1920w`,
